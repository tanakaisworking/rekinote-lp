      ...
      <div className="container-x">
        <div className="grid md:grid-cols-12 gap-10 lg:gap-12 items-center">
          ...
          <div className="md:col-span-5">
            ...
            {dict.compliance && dict.compliance.length > 0 && (
              <div className="mt-8 space-y-3">
                {dict.compliance.map((c) => (
                  <div key={c.acronym} className="flex items-start gap-3.5 p-4 rounded-xl panel-inset">
                    <span className="shrink-0 inline-flex items-center justify-center px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wider"
                          style={{ background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.4)", color: "#c4b5fd" }}>
                      {c.acronym}
                    </span>
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium leading-snug">{c.fullName}</div>
                      <div className="mt-1 text-[12.5px] muted leading-[1.7]">{c.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
