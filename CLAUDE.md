...
| フィールドID | 用途 | 備考 |
|-------------|------|------|
| `title_jp` | 日本語タイトル | 空なら日本語ページ非生成 |
| `title_en` | 英語タイトル | 空なら英語ページ非生成 |
| `description_jp` | 日本語概要 | 一覧・OGP用 |
| `description_en` | 英語概要 | 同上 |
| `body_jp` | 日本語本文 | リッチエディタ |
| `body_en` | 英語本文 | リッチエディタ |
| `eyecatch` | アイキャッチ画像 | 共通（言語問わず） |
| `category` | カテゴリ | コンテンツ参照。**英語マスタ**（Updates / Tips / Dev） |

旧フィールド `title` / `description` / `body`（サフィックスなし）は未使用。残っていてもフォールバックとして参照されるが、新規記事では `_jp` / `_en` を使うこと。
